...
class QuizViewModel : ViewModel() {
    ...
    fun loadQuizResults(userId: String, materialId: String, onLoaded: () -> Unit) {
        ...
        db.collection("quizResults")
            ...
            .orderBy("timestamp", Query.Direction.DESCENDING) // Ambil hasil terakhir
            .limit(1)
            .get()
    ...
    fun submitQuizManually(userId: String?, materialId: String) {
        if (userId != null && _userAnswers.isNotEmpty()) {
            Log.d("QuizViewModel", "Submit manual hasil kuis ke Firestore")
            saveQuizResultsToFirestore(userId, materialId)
        }
    }
